    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-0 md:p-4">
          <motion.div
            initial={{
              scale: 0.9,
              opacity: 0,
            ...
            className="relative w-full h-full md:w-auto md:h-auto md:max-w-4xl md:max-h-[90vh] overflow-y-auto bg-white dark:bg-gray-900 md:rounded-2xl shadow-2xl"
          >
            {/* Close button */}
            <button
              onClick={onClose}
              className="fixed top-4 right-4 z-30 p-3 rounded-full bg-black/80 dark:bg-white/20 backdrop-blur-md hover:bg-black/90 dark:hover:bg-white/30 transition-all duration-200 text-white shadow-2xl"
            ...
            <div className="sticky bottom-0 left-0 right-0 bg-white/95 dark:bg-gray-900/95 backdrop-blur-md border-t border-gray-200 dark:border-gray-700 p-4">
              <div className="flex gap-3">
                <a
                  href={`https://wa.me/${person.phone.replace(/[\s-]/g, '')}`}
                  target="_blank"
