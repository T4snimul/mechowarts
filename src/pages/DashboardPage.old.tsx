...
import { isAdmin, ROLE_NAMES, getUserPermissions } from '@/utils/permissions';
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-gray-900' : 'bg-gradient-to-b from-purple-50/50 via-white to-gray-50'}`}>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        {isAdmin(user) && (
          <div className={`rounded-2xl p-6 mt-8 ${isDark ? 'bg-gradient-to-br from-purple-900/50 to-pink-900/50 border-purple-700' : 'bg-gradient-to-br from-purple-50 to-pink-50 border-purple-200'} shadow-sm border`}>
            ...
            <p className={`text-sm mb-4 ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>
              You have full administrator access with all permissions enabled.
            </p>
            <div className="grid md:grid-cols-2 gap-3">
              <button className="flex items-center gap-3 p-4 rounded-xl bg-white/80 dark:bg-gray-800/80 hover:bg-white dark:hover:bg-gray-800 transition-colors">
                <svg className="w-5 h-5 text-purple-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />
                </svg>
                <div className="text-left flex-1">
                  <p className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>Manage Users</p>
                  <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Assign roles & permissions</p>
                </div>
              </button>
              <button className="flex items-center gap-3 p-4 rounded-xl bg-white/80 dark:bg-gray-800/80 hover:bg-white dark:hover:bg-gray-800 transition-colors">
                <svg className="w-5 h-5 text-purple-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div className="text-left flex-1">
                  <p className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>Content Management</p>
                  <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Manage site content</p>
                </div>
              </button>
              <button className="flex items-center gap-3 p-4 rounded-xl bg-white/80 dark:bg-gray-800/80 hover:bg-white dark:hover:bg-gray-800 transition-colors">
                <svg className="w-5 h-5 text-purple-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div className="text-left flex-1">
                  ...
                  <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>View site statistics</p>
                </div>
              </button>
              <button className="flex items-center gap-3 p-4 rounded-xl bg-white/80 dark:bg-gray-800/80 hover:bg-white dark:hover:bg-gray-800 transition-colors">
                <svg className="w-5 h-5 text-purple-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div className="text-left flex-1">
                  <p className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>Site Settings</p>
                  <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Configure platform</p>
                </div>
              </button>
            </div>
            <div className="mt-4 p-3 rounded-lg bg-white/50 dark:bg-gray-800/50">
              <p className={`text-xs ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>
                <strong>Your Permissions:</strong> {getUserPermissions(user).join(', ').replace(/_/g, ' ')}
              </p>
            </div>
          </div>
        )}
